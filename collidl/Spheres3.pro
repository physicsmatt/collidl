...
pro make_single_bond_between,vertex1, vertex2,nvertices,edges,unbounddisc, inbounds, inprocess, bound
	...
	unbounddisc[vertex1]=var2-var2/abs(var2)
	var2=unbounddisc[vertex2]
	unbounddisc[vertex2]=var2-var2/abs(var2)	
...
function it_would_help_to_bond_nicely ,vertex1,vertex2,edges, unbounddisc, inbounds, inprocess,bound,outofboundsok
	...
	if ((unbounddisc[vertex1]*unbounddisc[vertex2] le 0) and (inprocess(vertex2) eq 0) and ((inbounds(vertex2) eq 1) or (outofboundsok eq 1))) then begin
		return,1
	end else begin
		return,0
	end
...
function it_would_help_me_to_butt_in_on, vertex1, vertex2,edges,unbounddisc, inbounds, inprocess,bound,outofboundsok
	nvertices=n_elements(inbounds)
	if ((unbounddisc[vertex1]*unbounddisc[vertex2] le 0) and (inprocess(vertex2) eq 0)) then begin
		return,1
	end else begin
		return,0
	end
...
function can_retract_from, vertex1, vertex2, edges, unbounddisc, inbounds, inprocess,bound,outofboundsok
	...
	if ((inbounds[vertex2] ne 1) or (try_to_find_a_mate_nicely_for(vertex2,nvertices,edges,unbounddisc, inbounds, inprocess,bound,outofboundsok) eq 1) or (try_to_find_a_mate_rudely_for(vertex2,nvertices,edges,unbounddisc, inbounds, inprocess,bound,outofboundsok) eq 1)) then begin
		inprocess[vertex1]=0
		return,1
	end 
...
function try_to_find_a_mate_rudely_for, vertex,nvertices,edges,unbounddisc, inbounds, inprocess,bound,outofboundsok
	...
	for n=edges[vertex], edges[vertex+1]-1	do begin
		if ((it_would_help_me_to_butt_in_on(vertex, edges[n],edges,unbounddisc, inbounds, inprocess,bound,outofboundsok) eq 1) and can_butt_in_on(vertex, edges[n],edges, unbounddisc,inbounds, inprocess,bound,outofboundsok) eq 1) then begin
			make_single_bond_between,vertex, edges[n],nvertices,edges,unbounddisc, inbounds, inprocess,bound
			return,1
		end
	endfor
...
pro try_to_find_mates_for,vertex,nvertices,edges,unbounddisc, inbounds, inprocess, bound
	if (inbounds[vertex] eq 1) then begin
		inprocess(vertex)=1
		while ((unbounddisc[vertex] ne 0) and (try_to_find_a_mate_nicely_for(vertex,nvertices,edges,unbounddisc, inbounds, inprocess,bound,0) eq 1)) do nothing=0
		while ((unbounddisc[vertex] ne 0) and (try_to_find_a_mate_rudely_for(vertex,nvertices,edges,unbounddisc, inbounds, inprocess,bound,0) eq 1)) do nothing=0
		while ((unbounddisc[vertex] ne 0) and (try_to_find_a_mate_nicely_for(vertex,nvertices,edges,unbounddisc, inbounds, inprocess,bound,1) eq 1)) do nothing=0
		while ((unbounddisc[vertex] ne 0) and (try_to_find_a_mate_rudely_for(vertex,nvertices,edges,unbounddisc, inbounds, inprocess,bound,1) eq 1)) do nothing=0
		inprocess(vertex)=0
	end
end
...
pro main,small=small,simu=simu,stay=stay,wait=wait
		...
		if (0 eq 0) then begin
			...
			bondcount=0L;
			inbounds[where((goodx gt 2*bondlength) and (goodx lt xs-2*bondlength) and (goody gt 2*bondlength) and (goody lt ys-2*bondlength))]=1;			
