...
pro main,big=big,simu=simu,stay=stay,wait=wait,twomicron=twomicron,unfilt=unfilt
	...
	;*****************************************************
		...
		do_bonds=0; whether to make the smotthed angle field
...
;		cd, "/home/angelscu/idl/SPHERES3/try"
;		cd, "/usr/temp/" ; make this your favorite data directory
...
; THE MASTER LOOP
		...
		for i=0,n_elements(fs)-1 do begin
			...
			DEFSYSV, '!max_recursion_level', 5L
...
; MASTER LOOP ENDS
		...
		for LongIndex=0L, n_elements(fs)-1 do begin
			printf,u,Summary_of_Data[0,LongIndex],Summary_of_Data[1,LongIndex],Summary_of_Data[2,LongIndex],Summary_of_Data[3,LongIndex],Summary_of_Data[4,LongIndex]," ",fs[LongIndex]
		endfor
		free_lun,u
