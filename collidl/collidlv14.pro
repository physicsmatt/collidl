...
;
;   In future versions, there will also be a similar switch in the displaying of images, so that small image
;   function can be restored to X-windows machines, while allowing Windows to operate on larger images than
;   pixmap memory allows.  Furthermore, try blocks will be in place to allow backwards compatibility for machines
;   to default from both pixmap and to closing images if memory is not available.
;
...
;          analysis and a theoretically infinite top end.
...
pro main,scale=scale,spheresize=sphere_diameter,simu=simu,stay=stay,wait=wait,twomicron=twomicron,unfilt=unfilt,noimage=noimage, onemicron=onemicron, hardsphere=hardsphere
    ...
    ;*****************************************************
       ...
       ;     /simu : sets the sphere-locator parameters to Dan Vega's simulation size (obsolete)
       ;     /twomicron : sets the parameters for 2-micron picture (obsolete)
       ;     /onemicron : sets the parameters for 1-micron picture (obsolete)
       ;     /wait : waits for 5 secs after each file processed
       ;     /noimage : gets the vertices from a user routine rather than from a file
       ;     /hardsphere : to use with the hardsphere simu
    ;*****************************************************
...
;          DEFSYSV, '!yss',1.0*max(data1[1,*])
 ...
 dataunfilt=Interpolate(dataunfilt,findgen(scale*imagesize[1])/scale,findgen(scale*imagesize[2])/scale,/grid)
         ...
         endelse
          ...
          ;this removes the need for the memory hogging data2.
...
; Why set this every time it runs?
         ...
         goodsize = size(data1)
         DEFSYSV, '!goodx',fltarr(goodsize[2]-1)
         DEFSYSV, '!goody',fltarr(goodsize[2]-1)
    goodsize=0
...
;      triangulate, !goodx, !goody, triangles, outermost, CONNECTIVITY = edges
    ...
    goody_ok=0
       ...
       print, nvertices
         ...
         bondsenergy=(bondsl-bondslength)^2.0
...
;      print,'Writing angle TIFF file'
       ...
       if (do_force eq 0) then begin
        ...
        ;boutermost=0
         ...
         ;Also down below, but these are large memory hogs, and may need to be removed for later
         ;code to work. **************************************************************
         btriangles=0
         bondsenergy=0
...
;ccorr
    ...
    if (Ccorr eq 0) then begin
       ...
       bondsangle=0  ;moved down below (in if) *************
       bondsx=0
       bondsy=0
