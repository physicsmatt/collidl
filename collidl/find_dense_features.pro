...
function fourier_bpass,data,dominant_radius
    ...
    tmp=max(s*sf,w)
    r=1./s(w)
    if debug_mode then print,'Radius (max):        ',r,' pixels'
    dominant_radius = r
...
;profile[index] = (profile[index] - feat_dat[3,n] ) / feat_dat[4,n]
    ...
    if debug_mode then begin
       ...
       t1 = systime(/seconds)
       print,"build average profile complete time: ",t1-t0
       endif
...
debug_mode = 0
    ...
    for i = 0,5 do begin
       ...
       if debug_mode then print,'deleting: lt',i
       if (w1_count gt 0) then begin
            feature_data[6,w1]=0
            if debug_mode then print,feature_data[*,w1]
            endif
       if debug_mode then print,'deleting: gt',i
       if (w2_count gt 0) then begin
            feature_data[6,w2]=0
            if debug_mode then print,feature_data[*,w2]
            endif
        endfor
    w3 = where(feature_data[6,*] gt 0.5, w3_count)
    if (w3_count gt 0) then begin
        if debug_mode then print,'deleted all of the following'
        if debug_mode then print,feature_data[*,w3]
...
pro remove_duplicates,f,tolerance
    ;this implementation should be faster, but is totally untested.
    ;f[0,0] = f[*,sort(f[1,*])]

    ;rescale so that tolerance is ONE pixel
    inv_tolerance = 1.0 / tolerance
    x = f[0,*] * inv_tolerance
    y = f[1,*] * inv_tolerance
    max_x = max(f[0,*])
    for i = 0, 0.5 do begin
       for j = 0,0.5 do begin
         xy_loc = round(x + i, /L64) + (max_x * round(y + j,/L64))
         f = f[*,uniq(xy_loc)]
         endfor
       endfor
end

pro remove_duplicates_too_slow,f,dominant_length
    ;this implementation has got to be slow as hell!
...
;print,x,y,iter,a[0:1]
            ...
            if (status ne 0) then begin
                if debug_mode then print,"did not converge:",n,status,x,y,a
                endif
            if (((a[0] - x)^2 + (a[1] - y)^2) gt 4*radius*radius) then begin
                feature_data[6,n] = 0.0
                if debug_mode then print,'wiped out.',n,a[0],a[1]
                endif
...
function find_dense_features, image
    debug_mode = 0
    xy_data = get_initial_xy(image,dominant_length)
    ;for debugging: next line is for isolating a problem area:
    ;xy_data = xy_data[*,where((xy_data[1,*] gt 1950) and (xy_data[0,*] gt 1950) and (xy_data[0,*] lt 5000))]
    ...
    for i=1,3 do begin
        if debug_mode then print,"average a[2:4] is :",mean(feature_data[2,*]),mean(feature_data[3,*]),mean(feature_data[4,*])
        if debug_mode then show_locations,image,feature_data
       ...
       if debug_mode then show_locations,image,feature_data
       if debug_mode then graph_errors,feature_data[0,*]
       if debug_mode then print, 'program "find_features" has located: ',(size(feature_data))[2], ' points.'

    remove_duplicates,feature_data,dominant_length/4.0
