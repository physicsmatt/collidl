...
;This version is not meant for general use, and should only be used to produce information to test pixel bias.  for these purposes
; savelocation is now always enabled, and robcor is switched off.
...
pro main,saveloc=saveloc,flip=flip,scale=scale,spheresize=sphere_diameter,simu=simu,stay=stay,wait=wait,twomicron=twomicron,unfilt=unfilt,noimage=noimage, onemicron=onemicron, hardsphere=hardsphere, filtered=filtered,dorefine=dorefine
...
;       if (keyword_set(saveloc)) then begin
 ...
 ;      endif
 if (keyword_set(dorefine)) then begin
 endif else begin
    data1=0
endelse
...
if(keyword_set(dorefine)) then begin
distfromcenter=6
blah=refine(data,data1,distfromcenter)
print, blah
endif
