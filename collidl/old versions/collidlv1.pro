...
pro try, newimg
         ...
         showimage,[[[newre d]],[[newgreen]],[[newblue]]],3,wcombined
...
pro main,scale=scale,spheresize=sphere_diameter,simu=simu,stay=stay,wait=wait,twomicron=twomicron,unfilt=unfilt,noimage=noimage, onemicron=onemicron, hardsphere=hardsphere
    ...
    ;*****************************************************
       ...
       ;     /twomicron : sets the parameters for 1-micron picture (obsolete)
...
;          DEFSYSV, '!yss',1.0*max(data1[1,*])
        ...
        data=Interpolate(data,findgen((1024/(n[1]*1.0))*n[1])/(1024/(n[1]*1.0)),findgen((1024/(n[2]*1.0))*n[2])/(1024/(n[1]*1.0)),/grid)
                if (keyword_set(unfilt)) then begin
 dataunfilt=Interpolate(dataunfilt,findgen((1024/(n[1]*1.0))*n[1])/(1024/(n[1]*1.0)),findgen((1024/(n[2]*1.0))*n[2])/(1024/(n[1]*1.0)),/grid)
             endif
            endif else begin
            ; scale by scale(variable)
        data=Interpolate(data,findgen(scale*n[1])/scale,findgen(scale*n[2])/scale,/grid)
                if (keyword_set(unfilt)) then begin
 dataunfilt=Interpolate(dataunfilt,findgen(scale*n[1])/scale,findgen(scale*n[2])/scale,/grid)
                endif
            endelse
        endif
...
;      smoothbangle1=interpolate(smoothbangle,x/4.0,y/4.0,/cubic)
         ...
         if (do_angle_histogram eq 'yes') then begin
          ...
          angle_histogram[0,i] = angle_histogram[0,i] + single_angle_histogram[60]
          endif
         angimg=readimage(0)
