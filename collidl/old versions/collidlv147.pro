...
pro main,scale=scale,spheresize=sphere_diameter,simu=simu,stay=stay,wait=wait,twomicron=twomicron,unfilt=unfilt,noimage=noimage, onemicron=onemicron, hardsphere=hardsphere, filtered=filtered
...
; Why set this every time it runs?
       ...
       if (imagesize[1]*imagesize[2] gt 4194304) then begin ;4194304 is 2048x2048
        ...
        ;to save memory, set it to the minimum requirements
         ...
         Common globalgood, goodx, goody         ;this way they don't have to be passed 1298371298 times.
...
;      triangulate, goodx, goody, triangles, outermost, CONNECTIVITY = edges
    ...
    goody_ok=0
       ...
       if (showdiscbefore eq 0) then begin
         ...
         find_unbound_disc,nvertices,edges,unbounddisc, inbounds,inprocess, bound, ButtInOnAble,goodx,goody
         inprocess=0
       ButtInOnAble=0
...
if (do_postscript_defects eq 'yes') then begin
       openw,angle_histogram_unit,strmid(fs[0],0,strlen(fs[0])-4)+'postscript_defects.dat',/get_lun
       printf,format='(60I)',angle_histogram_unit,78
...
if (ev.TYPE eq 3) then tv, IMAGE[ev.x:ev.x+399, ev.Y:ev.y+399]
...
wDraw = widget_draw(tlb, X_SCROLL_SIZE=400, Y_SCROLL_SIZE=400, $
    XSIZE=8192, YSIZE=8192, /APP_SCROLL)

widget_control, tlb, /REALIZE
widget_control, wDraw, GET_VALUE=drawID
wset, drawID
tv, IMAGE[0:399,0:399]
xmanager, 'draw_app_scroll2', tlb, /NO_BLOCK
