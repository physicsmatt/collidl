...
pro paraboloid_pro,indep,params,F,dfda  ;written according to how it's required for "curvefit"
  ...
  f = A * (r2iw2*r2iw2 - r2iw2 + 1) + C
  dfda[0,0] = A *(2*dx*iw2 - 4*dx*r2iw2*iw2)        ;d/dx0
  dfda[0,1] = A *(2*dy*iw2 - 4*dy*r2iw2*iw2)       ;d/dy0
  dfda[0,2] = A *(2*r2iw2*iw - 4*r2iw2*r2iw2*iw)        ;d/dw
  dfda[0,3] = (r2iw2*r2iw2 - r2iw2)+1                      ;d/dA
  dfda[*,4] = 1.0                                          ;d/dC
  return
...
;  dy = y-y0
  ...
  r=sqrt((dy^2)+(dx^2))
  mrwp = -(iw*r)^p
  mask = (mrwp gt -87.0)  ; this little trick avoids a floating point underflow in the following exp() call
  exp_mrwp= mask * exp(mrwp * mask)
...
xarr_size = 64
yarr_size = 64
base_period = 4*(1.0 + 1.0/(xarr_size-20))
base_period = 4.1
;base_period = 5.01
amplitude = 80
offset = 128
...
amplitude = 80
offset = 128
noise_amplitude_multiplier = 0.01
...
;    feature_data[4,*]=127.9
    ...
    for i=1,2 do begin
      print,"average a[2] is :",mean(feature_data[2,*])
      print,"average a[3] is :",mean(feature_data[3,*])
       print,"average a[4] is :",mean(feature_data[4,*])
       show_locations,image,feature_data

       print,'refining'
;        refine10,image,feature_data,dominant_length/2.0
        refine10,image,feature_data,2.0/mean(feature_data[2,*])
       print,'done refining'
       ;show_locations,image,feature_data
