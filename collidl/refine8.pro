...
pro refine7, image, feature_data, distance_from_center
common shared_xypoints, xpoints, ypoints
    ...
    minxy = ceil(distance_from_center)
...
;            Brightnesses = (bilinear(float_image,xpoints,ypoints))[points]
            ...
            Best_values = CURVEFIT(points, Brightnesses, weights, a, chisq=chisq,FITA = [1,1,1,1,1],FUNCTION_NAME = 'sigmoid_2d_pro',iter=iter, $
                                status = status,itmax=100,/double)
            convergence = 1 - status
            feature_data[0,n]=a  ;this fills in valus 0 through 4
            feature_data[5,n]=chisq
...
print,'refining'
       ;refine7,image,feature_data,1.3*r_values[profile_size-1]
       refine7,image,feature_data,r_values[profile_size-1]
...
;show_locations,image,feature_data
hist_data = reform(feature_data[0,*]-floor(feature_data[0,*]))
plot,histogram(hist_data,binsize=0.05)
oplot,histogram(feature_data[1,*]-floor(feature_data[1,*]),binsize=0.02),color=128
    endfor
...
pro circle_areas,x0_in,y0_in,r
