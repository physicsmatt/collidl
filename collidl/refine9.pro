...
;  outbound=fltarr(6); return goodie box!
  dfda=dblARR(N_ELEMENTS(indep),5,/nozero)
...
xarr_size = 4096
yarr_size = 4096
base_period = 5 + 1.0/(xarr_size-20)
;base_period = 5.01
amplitude = 80
offset = 128
...
pro refine9_somewhat_old, image, feature_data, distance_from_center
common shared_xypoints, xpoints, ypoints

t0 = systime(/seconds)
    arr_size = ceil(distance_from_center) * 2 + 1
    xpoints = dblarr(arr_size,arr_size)
    ypoints = dblarr(arr_size,arr_size)
    weights = dblarr(arr_size,arr_size)
    xbase = (findgen(arr_size) - ceil(distance_from_center) + 0)#replicate(1.0,arr_size)
    ybase = replicate(1.0,arr_size) #(findgen(arr_size) - ceil(distance_from_center) + 0)
...
;            half_w = 0.5
;            weights[where(distance lt (distance_from_center-half_w))] = 1.0
;            weights[where(distance gt (distance_from_center+half_w))] = 0.0
;            w5 = where((distance ge (distance_from_center-half_w)) and (distance le (distance_from_center+half_w)))
;            weights[w5] = distance_from_center - distance[w5] + half_w
...
t5 = systime(/seconds)
print,"profile times:",t1-t0,t2-t1,t3-t2,t4-t3,t5-t4

show_locations,image,feature_data

print,'refining'
       refine9,image,feature_data,r_values[profile_size-1]+1
...
pro find_features, image
    ...
    for i=0,1 do begin
       ...
       print,'refining'
        refine9,image,feature_data,dominant_length/2.0
       print,'done refining'
       show_locations,image,feature_data
        weed_out_bad_points,image,feature_data
;show_locations,image,feature_data
...
common shared_xypoints, xpoints, ypoints
    ...
    t0 = systime(/seconds)
    arr_size = ceil(distance_from_center) * 2 + 2
    xpoints = dblarr(arr_size,arr_size)
    ypoints = dblarr(arr_size,arr_size)
    weights = dblarr(arr_size,arr_size)
    x_base = (findgen(arr_size) - ceil(distance_from_center) + 0)#replicate(1.0,arr_size)
    y_base = replicate(1.0,arr_size) #(findgen(arr_size) - ceil(distance_from_center) + 0)
...
;    for n= 239, 241 do begin
        ...
        if ((x GE minxy) and (y ge minxy) and (x le maxx) and (y le maxy)) then begin
            ...
            weights = dblarr(arr_size,arr_size)
            x_center = x-floor(x)+ceil(distance_from_center)
            y_center = y-floor(y)+ceil(distance_from_center)

            circle_areas,x_center,y_center,distance_from_center,weights
...
;  print, "big trouble", x,y,distance_from_center,left_side,right_side
;  endif
            use_points = where(weights gt 0.0)
            use_full_brightnesses=full_brightnesses[use_points]
            use_weights=weights[use_points]
            ...
            feature_data[5,n]=chisq
            feature_data[6,n]=convergence
            if (((a[0] - x)^2 + (a[1] - y)^2) gt 4*distance_from_center*distance_from_center) then begin
                feature_data[6,n] = 0.0
                print,'wiped out.',n
                endif
;print,n,convergence,iter,chisq,x,y, a
