...
READS, temp, a[0]
READ, Prompt='Imagined Center for test (y)?'
READS, temp, a[1]
...
READS, temp, a[4]
image=fltarr(x,y)
for j = 0, x do begin;indicies for pixels in area around previously found center
        for k=0,y do begin
        m=DCOMPLEX(j,k)
          b=sintest(m,a)
               image[j,k]=b[0]
...
function refineB, image, location_data, distance_from_center
...
for i = 0L, locsize[1]-1 do begin
    ...
    for j = -distance_from_center + round(x), distance_from_center + round(x) do begin;indecies for pixels in area around previously found center
        ...
        if j GE 0 and j LT pix[1] then begin
            if k GE 0 and k LT pix[2] then begin;populates an array for the brightnesses.  This should be good.
